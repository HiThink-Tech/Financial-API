# `hithink-finance futures contract-positions`

## 前置条件

- 按需读取[本域入口](../SKILL.md)与[共享规则](../../hithink-finance-shared/SKILL.md)，同会话已加载内容可复用。
- 首次执行或版本变化时用 `hithink-finance schema futures.contract-positions --format json` 确认参数，未说明的组合规则再看命令 `--help`。
- 远端调用需要 API Key，先按共享规则复用已有凭据。

## 命令

```bash
hithink-finance schema futures.contract-positions --format json
hithink-finance futures contract-positions --thscode <code> --variety <code> --date <date> --format json
```
