...
本页是同花顺金融数据服务 3 个 MCP 端点、22 个工具的固化功能契约，用于 Agent 的意图识别和工具路由。当前连接的 `tools/list` 用于确认实际可用性与调用 schema，不应取代本页的任务语义。
...
- 当前快照覆盖 A 股和 A 股指数/板块，不覆盖分钟 K、tick、Level-2、港股、美股、基金或期货。
