...
旧项目 `.env` 仍可留在仓库根，但新的统一凭据不放在项目目录。推荐使用用户级 `HITHINK_FINANCE_API_KEY` 或 Skill 管理的用户级 `hithink-finance/credentials.env`；`API_KEY`、`FUYAO_TOKEN` 仅作为兼容来源，不需要复制到 `python/`，也不得写入命令参数或提交。
