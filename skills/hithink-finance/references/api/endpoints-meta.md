...
| `asset_type` | query | string | 否 | 资产类别过滤，枚举 `a-share` / `a-share-index`。 | — |
...
| `asset_type` | string | 资产类别（`a-share` / `a-share-index`）。 |
...
| `asset_type` | query | string | 否 | 资产类别，枚举 `a-share` / `a-share-index`。 | `a-share` |
