...
└── fuyao/              扶摇远端 API toolkit（实时数据 / 财报）
```
...
| 增量补当天数据 | `marketdb update-daily` | marketdb（背后调 fuyao REST） |
| 全量重建 DB | `./bootstrap.sh` | 仓库根 |
| 拉某只票的最新快照 | `fuyao.py prices-snapshot --thscodes 300033.SZ` | fuyao |
| 拉某只票的财报 | `fuyao.py financials-income --thscode ...` | fuyao |
| 把名字翻成 thscode | `fuyao.py tickers-search --q "贵州茅台"` | fuyao |
| 查交易日历 | `fuyao.py calendar-trading-days` | fuyao |
...
1. **"历史 + 已对账的数据"永远走 marketdb**
   - 已经落入 `data/market.duckdb` 的 10 年日 K 是权威事实表（`raw_kline_daily`），不要再去打扶摇 REST 拉同一段时间的数据。
   - 复权因子用 `calc_adjust_factor_daily`（本地一次性算好），不要每次都重新算 / 重新拉事件。

2. **"实时 / 当天 / 财报 / 元数据"永远走 fuyao**
   - 当天盘后的最新快照、最新一期财报、刚刚刷新的 thscode 列表 —— 这些本地没有，必须走 fuyao。
   - 例外：`marketdb update-daily` 会**内部**调 fuyao REST 把新一两天的日 K 补进本地。这是唯一一处"marketdb → fuyao"的内部调用。

3. **fuyao 不直接落地大文件**
   - fuyao 的 CLI 只产 JSON 到 stdout。要落地 parquet / 入 marketdb，调用方负责（用 pandas 转一下，再走 `marketdb import-parquet`）。
   - 一次性大全量（10 年 + 全市场）不要打 fuyao —— 用 [扶摇 market dumps](https://fuyao.aicubes.cn/docs/api-reference/market-dumps/) 下 parquet，丢进 `refer-to/data/`，跑 `./bootstrap.sh`。
...
5. **不重复造文档**
   ...
   - fuyao 那边只描述协议 + 字段语义（见 `toolkit/fuyao/docs/llms-full.txt`）。
...
### 范式 B：用 fuyao 拿元数据 + 本地查行情

```bash
# 用 fuyao 把"贵州茅台"翻成 thscode
...
# 不要用 fuyao REST —— 直接去 market dumps 下 parquet
# 放进 refer-to/data/，然后：
./bootstrap.sh         # 自动检测新快照
```
...
| --- | --- |
| `marketdb` | ❌ 纯本地，不需要 |
| `fuyao` | ✅ `export FUYAO_TOKEN=<token>`，在 https://fuyao.aicubes.cn/admin 签发 |
| `marketdb update-daily` / `sync-symbols` | ✅ 走 `.env` 里的 `API_KEY`（同 fuyao token） |
