...
- 新增公开期货会话时间轴、品种板块、合约目录、主连、主力、次主力和商品指数命令。
- 新增公开期权会话时间轴和合约目录命令，并与 Python toolkit 保持契约一致。
...
- 新增 13 个期货命令和 4 个期权命令，覆盖公开品种、合约、持仓、仓单、基差、日程和行情。
- 标的搜索与列表新增 `futures`、`options` 资产类型，配套 Skills 与能力清单同步生成。

## 0.1.9 - 2026-09-09

- 新增基金在线回测、回测指标目录、画线/表格指标与 QDII 额度 6 项远端能力。
- 复杂 JSON 参数在发起 HTTP 请求前校验，结果保留动态字段、数值字符串和局部空值。
